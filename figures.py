...
for maxstreak in range(1, 7):
    ...
    for noRepeatRounds in range(1, 7):
        ...
        for teamSize in teamSizes:
            currentData = results[
                f"maxStreak={maxstreak};noRepeatRounds={noRepeatRounds}"
            ][f"teams={teamSize}"]

            alldrrv.append(
                averageNumberViolations(currentData["doubleRoundRobinViolations"])
            )
            mindrrv.append(
                minimumNumberViolations(currentData["doubleRoundRobinViolations"])
            )
            maxdrrv.append(
                maximumNumberViolations(currentData["doubleRoundRobinViolations"])
            )
        ...
        ys = drrvfit_fn(xs)
        (fit1,) = ax.plot(
            xs, ys, "--", color="salmon", label=rf"${coefsToLatex(drrvfit_fn.coef)}$"
        )
        ys = msvfit_fn(xs)
        (fit2,) = ax.plot(
            xs,
            ...
            label=rf"${coefsToLatex(msvfit_fn.coef)}$",
        )
        ys = nrvfit_fn(xs)
        (fit3,) = ax.plot(
            xs, ys, "--", color="gold", label=rf"${coefsToLatex(nrvfit_fn.coef)}$"
        )
    ...
    plt.savefig(
        f"figures/2026-04-10-increasing-no-repeat-maxstreak-of-{maxstreak}.png", dpi=250
    )
    plt.clf()
